...
output$info_adult_self_assessed_health = renderUI({
  ...
  tagList(
    ...
    h4("Adult (16+) self assessed health"),
...
output$info_screening = renderUI({
  ...
  tagList(
    ...
    bsButton("info_to_screening", "Go to green jobs tab")
...
output$info_infant_mortality_pop = renderUI({
  ...
  tagList(
    ...
    bsButton("info_to_infant_mortality_pop", "Go to infant mortality tab")
...
output$info_healthy_birthweight = renderUI({
  ...
  tagList(
    ...
    h4("Healthy birthweight babies"),

    br(),

    bsButton("info_to_healthy_birthweight", "Go to healthy birthweight babies tab")
...
output$info_children_at_risk_of_obesity = renderUI({
  ...
  tagList(
    ...
    bsButton("info_to_children_at_risk_of_obesity", "Go to child obesity tab")
...
##############################################.
# FIRST EVER HOSPITAL ADMISSIONS FOR A HEART ATTACK ----
##############################################.

output$info_heart_attack = renderUI({
  ...
  tagList(
    ...
    bsButton("info_to_heart_attack", "Go to hospital admissions for heart attack tab")
...
output$info_drug_admissions = renderUI({
  ...
  tagList(
    ...
    bsButton("info_to_drug_admissions", "Go to drug-related hospital admissions tab")
...
output$info_alcohol_admissions = renderUI({

  tagList(

    h4("Alcohol admissions"),

    br(),

    bsButton("info_to_alcohol_admissions", "Go to alcohol admissions tab")
...
output$info_asthma_hospital_adm = renderUI({

  tagList(

    h4("Asthma hospital admissions"),

    br(),

    bsButton("info_to_asthma_hospital_adm", "Go to asthma hopsital admissions tab")
...
output$info_drug_deaths = renderUI({
  ...
  tagList(
    ...
    bsButton("info_to_drug_deaths", "Go to drug related deaths tab")
...
output$info_alcohol_deaths = renderUI({

  tagList(

    h4("Alcohol specific deaths (aged 45-74)"),

    br(),

    bsButton("info_to_alcohol_deaths", "Go to alcohol-specific deaths tab")

  )

})

############## SOCIAL CARERS SURVEY ##########################----
...
##############################################.
# JE DASHBOARD UI FOR INFO NOTES ----
##############################################
...
} else if(input$indicator_type_info == "Adult risk of health") {

  wellPanel(



           h3("Adult risk of health"),
           bsCollapse(id = "info_collapse_adult_health", open = "Panel 1",
                      bsCollapsePanel(
                        "Adult (16+) self-assessed health",
                        uiOutput("info_adult_self_assessed_health")

                      ),
                      bsCollapsePanel(
                        "Gender pay gap",
                        uiOutput("info_adult_living_limiting_long_term_condition")
