...
observeEvent(input$chd_deaths_geog_name,{
  ...
  output$chd_deaths_title <- renderUI({h3(glue("Data table: Age-sex standardised rates per 100,000 of CHD deaths (under 75) in ",
                                              input$chd_deaths_geog_name))})
})
...
observeEvent(input$alcohol_admissions_geog_name,{
  ...
  output$alcohol_admissions_title <- renderUI({h3(glue("Data table: Total number of alcohol related admissions in ",
                                                      input$alcohol_admissions_geog_name))})
})
...
observeEvent(input$asthma_admissions_breakdowns,{
  observeEvent(input$asthma_admissions_geog_name,{
    ...
    output$asthma_admissions_title <- renderUI({h3(glue("Data table: Total number of asthma admissions ",
                                                       breakdown, "in ", geog))})
  })
})
