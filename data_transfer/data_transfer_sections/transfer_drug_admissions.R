...
drug_admin_age_sex_source<-read_csv(paste0(path_in_pop,"drug_admissions_age_sex.csv"))

  drug_admin_age_sex<-drug_admin_age_sex_source %>%
  clean_names() %>%
  ...
  drug_admin_age_sex_count<-drug_admin_age_sex %>%
    filter(measure!="Rate")%>%
    rename("count" = "value") %>%
    select(-measure)

  drug_admin_age_sex_rate<-drug_admin_age_sex %>%
    filter(measure!="Number")%>%
    rename("rate" = "value") %>%
    select(-measure)

  drug_admin_agesex<-drug_admin_age_sex_count %>%
    left_join(drug_admin_age_sex_rate, by=c("financial_year","hospital_type",
  ...
  rm(drug_admin_age_sex_source, drug_admin_age_sex, drug_admin_age_sex_count, drug_admin_age_sex_rate)

replace_file_fn(drug_admin_agesex, paste0(path_out, "/drug_admin_agesex.rds"))
